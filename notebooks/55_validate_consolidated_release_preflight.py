...
jobs_payload = w.api_client.do(
    "GET",
    "/api/2.2/jobs/list?limit=100",
)

job_by_name = {
    (job.get("settings") or {}).get("name"):
        job
    for job in jobs_payload.get(
        "jobs",
        [],
    )
}
...
try:
    ...
    app = None
    errors.append(
        "Databricks App could not be read: "
        + str(exc)
    )
...
for table_name in CORE_TABLES:
    found = spark.catalog.tableExists(
        table_name
    )
...
for table_name in INDEXED_GOVERNED_TABLES:
    ...
    if not found:
        warnings.append(
            "Governed corpus table not indexed yet: "
            + table_name
        )
...
for source_layer, root in SOURCE_ROOTS.items():
    ...
    if not found:
        errors.append(
            "Missing governed source root: "
            + source_layer
            + " → "
            + root
        )

shield_root = os.path.join(
    SOURCE_ROOTS[
        "IKF_INPUT"
    ],
    "SHIELD",
...
if not shield_exists:
    warnings.append(
        "Persistent SHIELD folder is not available yet: "
        + shield_root
    )
...
try:
    ...
    with driver.session() as session:
        ...
        rows.append(
            {
                ...
                "item":
                    "REFERENCE_CONTEXT not stored as ordinary SourceDocument",
                "status":
                    (
                        "PASS"
        ...
        if accidental_reference_nodes:
            errors.append(
                "REFERENCE_CONTEXT documents were found in the ordinary "
                "SourceDocument catalogue."
            )
        ...
        rows.append(
            {
                "area":
                    "Source-layer boundary",
                "item":
                    "SHIELD excluded from ordinary available catalogue",
                "status":
                    (
                        "PASS"
        ...
        if accidental_shield_active:
            errors.append(
                "SHIELD SourceDocument entries remain AVAILABLE in the "
                "ordinary document catalogue."
            )
...
if warnings:
    print(
        "PASS with setup warnings. Complete the listed indexing steps "
        "before feature-specific validators."
    )
else:
    print(
        "All checked release prerequisites are present."
    )
