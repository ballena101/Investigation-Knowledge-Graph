"""Databricks App bootstrap for shared IKF policy and UI modules.
...
1. repository checkout: ``app/`` beside ``src/`` — the strict transitional
   policy and UI source transformations are applied in memory;
...
The generated bundle is the preferred deployment source because policy and UI
adoption are then validated before any Databricks App runtime is used.
"""
